...
Figures regenerated by `make figures` for `RESULTS.md`:

| File | Content | Script |
|---|---|---|
| `detuning_regression` | the registration regression, EnCodec 3 kbps | `analysis/analyze_detuning.py` |
| `registration_all` | phase against detuning for every codec | `analysis/make_figures.py` |
| `residual_shape` | the residual over one semitone, per condition | `analysis/make_figures.py` |
| `rate_scaling` | bias against bitrate with the bypass floor | `analysis/make_figures.py` |
| `octaves` | the four-octave sweep | `analysis/make_figures.py` |
| `mechanism`, `overview` | residual on tones and real music; summary panel | `analysis/make_figures.py` |
| `pitch_histograms` | within-semitone F0 density of GTZAN and flattened GTZAN | `analysis/plot_histograms.py` |
